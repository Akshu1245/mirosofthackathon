...
export async function processDueSchedules(): Promise<SchedulerTickResult> {
  ...
  for (const row of due) {
    ...
    try {
      ...
      const nextSafe = new Date(Math.max(next.getTime(), Date.now() + MIN_INTERVAL_MS));
...
export function startRedTeamScheduler(intervalMs: number = 60_000): SchedulerLoopHandle {
  let stopped = false;
