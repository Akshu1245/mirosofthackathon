...
import type { CopilotMessageRow } from "@rakshex/database";

export type ExtendedIntentName =
  "wow_regressions" | "shadow_drift" | "custom_date_range" | "follow_up";
...
export function parseDateRange(query: string, now: Date = new Date()): DateRange | null {
  ...
  const between = q.match(/between\s+(\d{4}-\d{2}-\d{2})\s+(?:and|to|-)\s+(\d{4}-\d{2}-\d{2})/i);
  if (between) {
...
export function computeWowRegressions(
  ...
  const inPriorWeek = (d: Date): boolean => d.getTime() >= twoWeeksAgo && d.getTime() < oneWeekAgo;
