...
describe("evaluateRule", () => {
  ...
  it("requires ALL conditions to match (logical AND)", () => {
    ...
    const onlyOne = evaluateRule(rule, [snap("cost_usd", 20), snap("blocked_requests", 0)], NOW);
    expect(onlyOne.fired).toBe(false);
    const both = evaluateRule(rule, [snap("cost_usd", 20), snap("blocked_requests", 7)], NOW);
  ...
  it("handles all 5 operators correctly", () => {
    const matrix: Array<{
      op: AlertRule["conditions"][0]["operator"];
      thresh: number;
      val: number;
      expected: boolean;
    }> = [
  ...
  it("formats error_rate as percentage and latency as ms", () => {
    ...
    const v = evaluateRule(rule, [snap("error_rate", 0.42), snap("latency_p95_ms", 1234)], NOW);
