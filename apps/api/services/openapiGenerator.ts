...
export function zodToJsonSchema(zod: ZodTypeAny, _opts: ConvertOpts = {}): JsonSchema {
  ...
  switch (kind) {
    ...
    case "ZodTuple":
    case "tuple": {
      const items = (def as unknown as { items?: ZodTypeAny[] }).items ?? [];
      if (items.length === 0) return { type: "array", items: { "x-any": true } };
      return {
        type: "array",
...
export function walkRouter(router: AnyTRPCRouter): ProcedureEntry[] {
  ...
  if (def?.procedures) {
    for (const [path, proc] of Object.entries(def.procedures)) {
      ...
      const type = (procDef?.type ?? "query") as "query" | "mutation" | "subscription";
      out.push({
        path,
        procedure: proc,
        type,
...
function walkRecord(record: Record<string, unknown>, prefix: string, out: ProcedureEntry[]): void {
    ...
    if (childDef.type === "query" || childDef.type === "mutation") {
      ...
      for (const [k, p] of Object.entries(childDef.procedures)) {
        ...
        out.push({
          path: `${path}.${k}`,
          procedure: p,
          type: (subDef?.type ?? "query") as "query" | "mutation" | "subscription",
...
export function buildOpenApiSpec(router: AnyTRPCRouter, opts: BuildSpecOptions): OpenApiSpec {
  ...
  for (const entry of procedures) {
    ...
    const security = entry.isProtected && !isPublic ? [{ cookieAuth: [] }] : undefined;
    ...
    if (entry.type === "query") {
        ...
        op.parameters = [
          {
            ...
            required: true,
            schema: inputSchema,
            description: "URL-encoded JSON of the procedure input. tRPC v11 wire format.",
          },
        ];
      }
    } else {
      op.requestBody = {
        ...
        content: {
          "application/json": {
            schema: inputSchema ?? ({ type: "object", additionalProperties: false } as JsonSchema),
          },
        },
      };
    }
