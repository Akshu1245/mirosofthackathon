...
describe("parseDateRange", () => {
  ...
  it("parses 'between YYYY-MM-DD and YYYY-MM-DD'", () => {
    const r = parseDateRange("show me between 2026-04-01 and 2026-04-15", FIXED_NOW);
  ...
  it("rejects backward ranges", () => {
    expect(parseDateRange("between 2026-04-15 and 2026-04-01", FIXED_NOW)).toBeNull();
  });
...
describe("computeWowRegressions", () => {
  ...
  it("returns null pctChange when prior week is zero", () => {
    ...
    expect(signals.find((s) => s.signal === "blocked_attempts")!.pctChange).toBeNull();
  });
});
...
describe("detectFollowUp", () => {
  ...
  it("returns isFollowUp=false for self-contained queries", () => {
    expect(detectFollowUp("show me the latest red-team score", messages as never).isFollowUp).toBe(
      false,
    );
  });
