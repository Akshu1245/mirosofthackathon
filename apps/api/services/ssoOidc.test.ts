...
describe("buildAuthorizeUrl", () => {
  it("emits a well-formed authorize URL with PKCE", () => {
    ...
    expect(u.searchParams.get("redirect_uri")).toBe("https://app.example.com/auth/sso/1/callback");
...
describe("decodeIdToken", () => {
  ...
  it("rejects tokens missing required claims", () => {
    expect(() => decodeIdToken(makeIdToken({ iss: "x", aud: "y" }))).toThrow(/'sub' claim/);
    expect(() => decodeIdToken(makeIdToken({ sub: "x", aud: "y" }))).toThrow(/'iss' claim/);
    expect(() => decodeIdToken(makeIdToken({ sub: "x", iss: "y" }))).toThrow(/'aud' claim/);
  });
});
