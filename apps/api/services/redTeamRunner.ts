...
import { INJECTION_PAYLOADS, type InjectionPayload } from "../utils/promptInjectionPayloads";
import { createRedteamRun, getRedteamRun, recordRedteamFindings, updateRedteamRun } from "../db";
...
export async function runRedTeam(opts: RedTeamRunOptions): Promise<RedTeamRunSummary> {
    ...
    workers.push(
      (async () => {
        while (cursor < universe.length) {
          ...
          const outcome = status === 0 ? "errored" : classifyResponse(payload, status, body);
  ...
  const securityScore = universe.length === 0 ? 100 : Math.round((blocked / universe.length) * 100);
