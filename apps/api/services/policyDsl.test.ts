...
import { POLICY_TEMPLATES, getPolicyTemplate } from "./policyTemplates";
import { PolicyValidationException, compilePolicy, parsePolicy } from "./policyDsl";
...
`;
  ...
  it("rejects non-object root with structured error", () => {
    ...
    try {
      ...
      expect((err as PolicyValidationException).errors[0].message).toMatch(/must be an object/);
    }
  });
  ...
  it("rejects malformed YAML with parse error", () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(PolicyValidationException);
      expect((err as PolicyValidationException).errors[0].message).toMatch(/invalid YAML/);
    }
  });
...
`);
      ...
      expect(err).toBeInstanceOf(PolicyValidationException);
      expect((err as PolicyValidationException).errors[0].message).toMatch(/unknown rule id/);
    }
  });
...
`);
      ...
      expect(err).toBeInstanceOf(PolicyValidationException);
      expect((err as PolicyValidationException).errors[0].path).toBe("rules[0].threshold");
    }
  });
...
describe("policy templates", () => {
  it("ships the 5 expected templates", () => {
    ...
    expect(ids).toEqual(["balanced", "demo-loose", "india-pii", "permissive", "strict"]);
  });
