...
describe("resolveDisplayFields", () => {
  it("uses the provided email when valid", () => {
    const r = resolveDisplayFields({ subject: "u1", email: "Alice@Example.com" }, 1);
  ...
  it("trims name whitespace", () => {
    const r = resolveDisplayFields({ subject: "u1", name: "  Bob  " }, 1);
    expect(r.name).toBe("Bob");
  });

  it("ignores invalid emails missing @", () => {
    const r = resolveDisplayFields({ subject: "u1", email: "not-an-email" }, 3);
